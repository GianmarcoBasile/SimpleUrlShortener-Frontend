...
export const Home = () => {
  ...
  function handleSubmit(e: SubmitEvent<HTMLFormElement>, url: string) {
    ...
    if (!shortened) {
      axios
        .post(`${BACKEND_URL}:${BACKEND_PORT}/url`, { originalUrl: url })
        .then((response) => {
          console.log("Shortened URL:", response.data);
          setShortenedUrl(
            `${BACKEND_URL}:${BACKEND_PORT}/url/${response.data.shortURL}`,
          );
          setShortened(true);
        })
        .catch((error) => {
    ...
    <>
      ...
      {shortenedUrl && (
        <div className="flex justify-center items-center pt-4">
          ...
          />
          {copied ? (
            <Button className="bg-green-500 text-white p-2 ml-2 cursor-pointer">
              Copied!
            </Button>
          ) : (
            <Button
              onClick={handleCopy}
              className="bg-green-500 text-white p-2 ml-2 cursor-pointer"
            >
              Copy
            </Button>
          )}
